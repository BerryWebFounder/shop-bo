export default defineEventHandler(async (event) => {
    ...
    const targetUrl = path ? `${config.public.usersApiBaseUrl}/admin/${path}` : `${config.public.usersApiBaseUrl}/admin`

    console.log(`[Admin Server API] ${method} ${event.node.req.url} -> ${targetUrl}`)
    ...
    try {
        ...
        console.error(`[Admin Server API] Error:`, error)
        if (error.response) {
