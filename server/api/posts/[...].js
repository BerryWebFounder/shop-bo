export default defineEventHandler(async (event) => {
    ...
    const targetUrl = path ? `${config.public.apiBaseUrl}/posts/${path}` : `${config.public.apiBaseUrl}/posts`

    console.log(`[Posts Server API] ${method} ${event.node.req.url} -> ${targetUrl}`)
    ...
    try {
        ...
        console.error(`[Posts Server API] Error:`, error)
        if (error.response) {
