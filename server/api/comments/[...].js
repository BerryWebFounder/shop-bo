export default defineEventHandler(async (event) => {
    ...
    const targetUrl = path ? `${config.public.apiBaseUrl}/comments/${path}` : `${config.public.apiBaseUrl}/comments`

    console.log(`[Comments Server API] ${method} ${event.node.req.url} -> ${targetUrl}`)
    ...
    try {
        ...
        console.error(`[Comments Server API] Error:`, error)
        if (error.response) {
