export default defineEventHandler(async (event) => {
    ...
    const targetUrl = path ? `${config.public.apiBaseUrl}/notices/${path}` : `${config.public.apiBaseUrl}/notices`

    console.log(`[Notices Server API] ${method} ${event.node.req.url} -> ${targetUrl}`)
    ...
    try {
        ...
        console.error(`[Notices Server API] Error:`, error)
        if (error.response) {
