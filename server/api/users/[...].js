export default defineEventHandler(async (event) => {
    ...
    const targetUrl = path ? `${config.public.usersApiBaseUrl}/users/${path}` : `${config.public.usersApiBaseUrl}/users`

    console.log(`[Users Server API] ${method} ${event.node.req.url} -> ${targetUrl}`)
    ...
    try {
        ...
        console.error(`[Users Server API] Error:`, error)
        if (error.response) {
